 ...
 * @version 1.0
 * <p>
 * Ver 1.0, 2019/10/26, Administrator, Create file
 */
public class SPUtil {
     ...
     * @return {@link SPUtil}
     */
    public static SPUtil getInstance(String spName, Context context) {
        if (isSpace(spName)) {
            spName = "gameSdk";
        }
        SPUtil sp = sSPMap.get(spName);
        if (sp == null) {
            sp = new SPUtil(spName, context);
    ...
    private SPUtil(String spName, Context context) {
        if (null == context) {
